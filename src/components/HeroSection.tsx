...
const HeroSection = () => {
  ...
  return (
    <section className="bg-gradient-to-r from-[#173e69] via-[#297585] to-[#57acc1] container-pattern text-white min-h-screen flex flex-col justify-between py-12 sm:py-16 md:py-20 overflow-y-auto">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex flex-col lg:flex-row items-center">
        ...
        >
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-3 sm:mb-4 tracking-tight leading-tight">
            Elevate Your{" "}
            <span className="text-teal-200 inline-block">Digital Presence</span>
          </h1>
          <p className="text-base sm:text-lg md:text-xl opacity-90 mb-6 sm:mb-8 max-w-xl">
            We create stunning websites and strategic social media campaigns
            that help your business connect with customers and grow online.
          </p>
          ...
          <div className="flex items-center gap-4 mt-8 lg:hidden">
            <div className="flex -space-x-2">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="w-8 h-8 rounded-full bg-gradient-to-r from-teal-300 to-blue-300 border-2 border-white flex items-center justify-center text-xs font-bold"
                >
                  {i}
                </div>
              ))}
      ...
      <motion.div
        className="hidden lg:flex items-center gap-4 mt-8 px-8 justify-start"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <div className="flex -space-x-3">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="w-10 h-10 rounded-full bg-gradient-to-r from-teal-300 to-blue-300 border-2 border-white flex items-center justify-center text-sm font-bold"
            >
              {i}
            </div>
          ))}
